...
local function expand_user_dir(path)
	if not path or path == "" then
		return nil
	end

	local home = os.getenv("HOME") or ""
	local expanded = path:gsub("^~", home):gsub("^%$HOME", home)
	...
	if opts.capture == "area" then
		...
		table.insert(command, "-window")
		table.insert(command, "root")

		if delay and delay > 0 then
			table.insert(command, "-delay")
			table.insert(command, tostring(delay))
		end
	...
	local message
	if opts.capture == "area" then
		message = string.format("Select region -> %s", filename)
	else
		message = string.format("Screenshot saved -> %s", filename)
	end
...
-- Create battery widget using vicious
mybattery = wibox.widget.textbox()
vicious.register(mybattery, vicious.widgets.bat, "🔋 $1$2% $3h", 30, "BAT0")
...
globalkeys = gears.table.join(
	...
	end, { description = "switch windows with rofi", group = "launcher" }),
	...
	awful.key({}, "Print", function()
		take_screenshot()
	end, { description = "screenshot with 9s delay", group = "system" }),
...
-- Enable tap-to-click for all touchpad devices
awful.spawn.once(
	"bash -c \"xinput list --id-only | while read id; do xinput list-props $id 2>/dev/null | grep -q 'libinput Tapping Enabled' && xinput set-prop $id 'libinput Tapping Enabled' 1 2>/dev/null || true; done\""
)
-- }}}
...
awful.spawn.once("xset r rate 220 25")
-- }}}
